...
export default function NewProjectPage() {
  ...
  const [accent, setAccent] = useState("oklch(0.6 0.12 250)")
  ...
  const autoGenerateId = () => {
    const titleZh = bilingual.title?.zh || ""
    const titleEn = bilingual.title?.en || ""
    const source = titleEn || titleZh
    if (source) {
      setId(slugify(source))
    }
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const project: Partial<Project> & { id: string; title: LocalizedString } = {
        id: id.trim(),
        year,
        accent,
        image,
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <div>
              <label className="mb-1.5 block text-sm font-medium">
                ID <span className="text-muted-foreground">(英文标识)</span>
              </label>
