...
try{
 const plan=JSON.parse(await readFile('seed-data/apricot-study.json','utf8'));
 if(plan.sourceKey!=='/project-apricot-app/')throw Error('Apricot source key required');
 ...
 if(JSON.stringify(descriptionImages(patch.description))!==JSON.stringify(patch.images)||patch.images.length!==15||patch.imageCaptions.length!==15)throw Error('Expected fifteen original image/text pairs');
 ...
 if(changed){
  if(hash(current)!==plan.expectedHash)throw Error('Apricot content edited; review before applying this upgrade');
  await mkdir('data/content-backups',{recursive:true});
  await writeFile(`data/content-backups/apricot-${Date.now()}.json`,JSON.stringify(current,null,2),{flag:'wx'});
  const result=await db.work.updateMany({where:{id:current.id,updatedAt:current.updatedAt},data:patch});
  if(result.count!==1)throw Error('Concurrent Apricot edit; no changes applied');
 }
 ...
 if(seedText!==next){
  ...
  const temp=`seed-data/apricot-${Date.now()}.tmp`;await writeFile(temp,next,{flag:'wx'});await rename(temp,'seed-data/content.json');
 }
 console.log(`Apricot study: ${changed?1:0} record changed; metadata preserved; first-install content synchronized.`);
