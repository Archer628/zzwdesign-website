...
try{
 const plan=JSON.parse(await readFile('seed-data/michaels-study.json','utf8'));
 if(plan.sourceKey!=='/project-michaels-self-checkout/')throw Error('Only Michaels Self Checkout may be updated');
 ...
 const sources=JSON.parse(await readFile('seed-data/michaels-study-sources.json','utf8'));
 ...
 if(changed&&hash(current)!==plan.expectedHash)throw Error('Michaels content edited; review before applying upgrade');
 ...
 if(changed){
  ...
  await writeFile(`data/content-backups/michaels-study-${Date.now()}.json`,JSON.stringify(current,null,2),{flag:'wx'});
 ...
 if(seedText!==next){
  ...
  const temp=`seed-data/michaels-study-${Date.now()}.tmp`;await writeFile(temp,next,{flag:'wx'});await rename(temp,'seed-data/content.json');
 }
 console.log(`Michaels study: ${changed?1:0} records changed; first-install content synchronized.`);
